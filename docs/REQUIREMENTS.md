...
| REQ-040 | **Build-Version enthält Build-Zeitstempel:** Die in `dist/sharkord-vid-with-friends/package.json` geschriebene Plugin-Version enthält einen Build-Postfix im Format `DDMMYY_HH_MM_SS` (z. B. `070326_15_04_09`) und wird loader-kompatibel als `<basisversion>-<DDMMYY_HH_MM_SS>` geschrieben. Zusätzlich wird ein lesbares Trace-Feld `sharkordVersionTrace` im Format `<basisversion>:<DDMMYY_HH_MM_SS>` geschrieben. Damit ist jeder Build eindeutig zeitlich rückverfolgbar, ohne Plugin-Loader zu brechen. | Must |
